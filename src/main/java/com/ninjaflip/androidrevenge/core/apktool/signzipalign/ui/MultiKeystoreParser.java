...
public class MultiKeystoreParser {
    ...
    List<Arg.SignArgs> parse(CommandLine commandLine) {
        if (commandLine.hasOption("ksDebug")) {
            ...
            List<Arg.SignArgs> signArgsList = new ArrayList<Arg.SignArgs>();
            ...
            if (!commandLine.hasOption("ks")) {
                ...
                Map<Integer, String> ksArgList = new HashMap<Integer, String>();
                Map<Integer, String> ksAliasArgList = new HashMap<Integer, String>();
                Map<Integer, String> ksPassArgList = new HashMap<Integer, String>();
                Map<Integer, String> ksKeyPassARgList = new HashMap<Integer, String>();
                ...
                //Java 6 code
                /*List<Arg.SignArgs> tmp = new ArrayList<Arg.SignArgs>();
                for (Map.Entry<Integer, String> entry : ksArgList.entrySet()) {
                    tmp.add(new Arg.SignArgs(entry.getKey(), entry.getValue(), ksAliasArgList.get(entry.getKey()),
                            ksPassArgList.get(entry.getKey()), ksKeyPassARgList.get(entry.getKey())));
                }
                signArgsList.addAll(tmp);*/
            }
