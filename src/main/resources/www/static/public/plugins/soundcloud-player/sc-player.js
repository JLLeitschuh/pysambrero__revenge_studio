...
function SoundCloudAPI(myApiKey) {
    ...
    function $panic(msg, errorback) {
        notify('SoundCloudAPI: ' + msg, "warning", 5000);
        if (_$.debug && window.console) {
            console.error('SoundCloudAPI: ' + msg);
            notify('SoundCloudAPI: ' + msg, 'error', 5000);
        }
...
function init_sc_music_player(myApiKey) {
    var SC = {
        'API': new SoundCloudAPI(myApiKey),
        'Global': false,
        'Volume': 0.8,
        'Tracks': {},
        'Object': {}
    ...
    var _Current_ = {
        ...
        connect: function (player_node, track_node) {
            ...
            $('#sc-equalizer').show();
            bringFocus($('#' + track_node.id));
            try {
                if (typeof(Storage) !== "undefined") {
                    localStorage.setItem('active_sc_tack', track_node.id);
                }
            } catch (e) {
                // do nothing
            }
        ...
        destroyAudio: function () {
            ...
            if (this['AudioDevice'] !== null) {
                this['AudioDevice'].src = '';
                this['AudioDevice'].load();
                this['AudioDevice'].play();
            }
            $('#sc-equalizer').hide();
        },

        onConnexionLost: function () {
            if (this['AudioDevice'] !== null) {
                this['AudioDevice'].src = '';
                this['AudioDevice'].load();
                this['AudioDevice'].play();
    ...
    function _scCreate(link) {
        ...
        SC['API'].getTracks(link.href, function (tracks) {
                ...
                if (typeof(Storage) !== "undefined") {
                    ...
                    if (active_sc_tack !== null) {
                        ...
                        if (active_track_item !== null) {
                            active_track_item.click();
                            bringFocus($('#' + active_track_item.id));
                        }
                    }
                }
            } catch (e) {
            ...
            $('#music-player-container').unblock();
        }.bind(exp), function (error) {
            this.node.removeAttribute('id');
        }.bind(exp));
    ...
    function _scDestroy() {
        _Current_.destroyAudio();
    }

    function _scOnConnexionLost() {
        _Current_.onConnexionLost();
    }
    ...
    function onEnd(e) {
        ...
        if ((play_next = _Current_.SelectTrack.nextElementSibling)) {
            ...
            _Current_.PlayerNode['_button_'].textContent = '';
            $('#sc-equalizer').hide();
    ...
    function onPlayerAction(e) {
        ...
        //console.log("player action "+ e.type);
        if (e.type === 'pause') {
            $('#sc-equalizer').hide();
            try {
                if (typeof(Storage) !== "undefined") {
    ...
    function createPlayerDOM(hash, len) {
        ...
        div.innerHTML = '<ol class="sc-artwork-list"></ol>\n' +
            ...
            '	<a href="#control" class="sc-play"></a>\n' +
            '</div>\n' +
            ...
            '	</div>\n' +
            '</div>' +
            '<div id="sc-equalizer"><img src="static/public/images/equalizer-anim-sm.gif" alt="" height="30px"/><div>';
        if (hash) {
            div.id = 'sc-obj_' + hash;
            if (!SC['Global']) {
    ...
    function updateTrackInfo(node, track) {
        ...
        node['_info_'].children[0].innerHTML = '<a target="_blank" href="' + track.permalink_url + '">' + track.title + '</a>';
        node['_info_'].children[1].innerHTML = 'by <a target="_blank" href="' + track.user.permalink_url + '">' + track.user.username + '</a>';
    ...
    function bringFocus(track_item) { // when start playing a track and track is not visible we bring focus to it with a scroll animation
    ...
    window.SCPurePlayer = {
        ...
        destroy: _scDestroy,
        onConnexionLost: _scOnConnexionLost,
        stopScPlaying: _scOnConnexionLost
    };
